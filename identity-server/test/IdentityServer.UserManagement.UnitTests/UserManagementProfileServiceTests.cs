...
namespace IdentityServer.UserManagement.UnitTests;
...
public sealed class UserManagementProfileServiceTests : IAsyncLifetime
{
    ...
    public async Task GetProfileDataAsync_emits_role_claims_for_direct_and_transitive_roles()
    {
        ...
        await _membershipAdmin.AssignRoleAsync(subjectId, adminRoleId, _ct);

        // Assign editor transitively via a group
        var groupId = await CreateGroup("editors");
        await _membershipAdmin.AssignGroupAsync(subjectId, groupId, _ct);
        await _membershipAdmin.AssignRoleToGroupAsync(editorRoleId, groupId, _ct);
    ...
    public async Task GetProfileDataAsync_deduplicates_roles_when_role_appears_both_directly_and_transitively()
    {
        ...
        await _membershipAdmin.AssignGroupAsync(subjectId, groupId, _ct);
        await _membershipAdmin.AssignRoleToGroupAsync(adminRoleId, groupId, _ct);
    ...
    public void DI_resolves_UserManagementProfileService_without_profiles_or_membership_registered()
    {
        ...
        profileService.ShouldBeOfType<UserManagementProfileService>();
    }
}
