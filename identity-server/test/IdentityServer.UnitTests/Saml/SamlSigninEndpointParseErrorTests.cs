...
public class SamlSigninEndpointParseErrorTests
{
    private const string Category = "SAML Signin Endpoint Parse Error";

    [Fact]
    [Trait("Category", Category)]
    public async Task parse_error_with_known_issuer_returns_validation_problem_result()
    {
        var extractor = new AlwaysFailSigninExtractor();
    ...
    [Fact]
    [Trait("Category", Category)]
    public async Task parse_error_returns_generic_message_not_exception_message()
    {
        var extractor = new AlwaysFailSigninExtractor();
        var endpoint = CreateEndpointWithExtractor(extractor);
        var context = CreatePostContextWithIssuer("https://sp.example.com");
        ...
        result.ShouldBeOfType<ValidationProblemResult>();
        result.ShouldNotBeOfType<SamlErrorResponse>();
    }
    ...
    {
        var realExtractor = new SamlSigninRequestExtractor(
            new AuthNRequestParser(NullLogger<AuthNRequestParser>.Instance));
    ...
    [Fact]
    [Trait("Category", Category)]
    public async Task parse_error_never_returns_a_saml_error_response()
    {
        var extractor = new AlwaysFailSigninExtractor();
        var endpoint = CreateEndpointWithExtractor(extractor);
        var context = CreatePostContextWithIssuer("https://sp.example.com");
    ...
    private sealed class AlwaysFailSigninExtractor()
        : SamlSigninRequestExtractor(new AuthNRequestParser(NullLogger<AuthNRequestParser>.Instance))
    {
        protected override AuthNRequest ParseRequest(XDocument xmlDocument) =>
    ...
    private static SamlSigninEndpoint CreateEndpointWithExtractor(SamlSigninRequestExtractor extractor) =>
        new(
            extractor: extractor,
            signinRequestProcessor: null!, // never called on parse error path
            logger: NullLogger<SamlSigninEndpoint>.Instance,
