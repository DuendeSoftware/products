...
public class DiagnosticHostedServiceTests
{
    ...
    public async Task ExecuteAsync_ShouldNotThrowWhenDelegatePropertiesAreSet()
    {
        ...
        identityServerOptions.Saml.AuthnRequestErrorInspector = _ => { };
        identityServerOptions.Saml.LogoutRequestErrorInspector = _ => { };
        identityServerOptions.Saml.LogoutResponseErrorInspector = _ => { };

        var entry = new IdentityServerOptionsDiagnosticEntry(Options.Create(identityServerOptions));
        var diagnosticService = new DiagnosticDataService(DateTime.UtcNow, [entry], new FakeTimeProvider());
