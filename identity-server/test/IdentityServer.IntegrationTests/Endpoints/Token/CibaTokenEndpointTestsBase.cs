...
public abstract class CibaTokenEndpointTestsBase
{
    private const string Category = "CIBA Token endpoint";
    private readonly Ct _ct = TestContext.Current.CancellationToken;

    protected IdentityServerPipeline _mockPipeline = new IdentityServerPipeline();
    protected FakeTimeProvider _clock = new FakeTimeProvider(DateTimeOffset.UtcNow);
    ...
    protected CibaTokenEndpointTestsBase()
    {
        _mockPipeline.OnPreConfigureServices += s => s.TryAddSingleton<TimeProvider>(_clock);
    ...
    protected void InitializePipeline() => _mockPipeline.Initialize();
    ...
    public async Task valid_request_should_return_valid_result()
    {
        ...
        var request = await cibaService.GetLoginRequestByInternalIdAsync(_mockCibaUserNotificationService.LoginRequest.InternalId, _ct);
    ...
    public async Task expired_request_should_return_error()
    {
        ...
        // token request
        _clock.SetUtcNow(DateTimeOffset.UtcNow.AddHours(1));
    ...
    public async Task calls_past_interval_should_reset_throttling()
    {
        ...
        _clock.Advance(TimeSpan.FromSeconds(_mockPipeline.Options.Ciba.DefaultPollingInterval));
    ...
    public async Task client_lifetime_should_be_honored()
    {
        ...
        _clock.Advance(TimeSpan.FromSeconds(_cibaClient.CibaLifetime.Value + 1));
