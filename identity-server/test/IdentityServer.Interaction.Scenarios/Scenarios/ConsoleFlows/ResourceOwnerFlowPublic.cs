...
/// <summary>
/// Scenario: Resource Owner Password Credentials flow with a public client (no client secret).
/// The client requests a token directly using the user's username and password, without authenticating itself.
/// </summary>
public sealed class ResourceOwnerFlowPublic : IScenario
{
    private IdentityServerTestHost? _identityServer;
    private ApiHost? _api;

    public string Name => "ResourceOwnerFlowPublic";
    public string Description => "Resource Owner Password Credentials (public client): obtain token with username/password (no client secret), call API";
    ...
    [
        new Command
        {
            Name = "Run Resource Owner Flow (Public Client)",
