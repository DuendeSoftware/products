...
{
    private const string EntityFrameworkStoreDescription = "Entity Framework storage";
    ...
    extension(IIdentityServerBuilder builder)
    {
            ...
            where TContext : DbContext, IConfigurationDbContext
        {
            ...
            // Entity Framework storage does not provide admin services. Register the admin
            // interfaces as throwing stubs so resolving them fails with a clear message instead
            // of a generic "no service registered" error. Admin services require AddStorage().
            builder.Services.DisableAdmin<IClientAdmin>(EntityFrameworkStoreDescription);
            builder.Services.DisableAdmin<IIdentityResourceAdmin>(EntityFrameworkStoreDescription);
            builder.Services.DisableAdmin<IApiResourceAdmin>(EntityFrameworkStoreDescription);
            builder.Services.DisableAdmin<IApiScopeAdmin>(EntityFrameworkStoreDescription);
            builder.Services.DisableAdmin<IIdentityProviderAdmin>(EntityFrameworkStoreDescription);
            builder.Services.DisableAdmin<ISamlServiceProviderAdmin>(EntityFrameworkStoreDescription);
        ...
        /// <returns>The <see cref="IIdentityServerBuilder"/> for chaining.</returns>
        public IIdentityServerBuilder AddOperationalStoreNotification<T>()
