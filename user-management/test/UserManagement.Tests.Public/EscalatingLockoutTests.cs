...
public sealed class EscalatingLockoutTests
{
    ...
    private static async Task<(ServiceProvider ServiceProvider, IPasswordAuth PasswordAuth, IUserAuthenticatorsSelfService SelfService, IUserSelfService UserSelfService, FakeTimeProvider TimeProvider)> CreateAsync(
        Action<UserAuthenticationOptions> configure)
    {
        ...
        return (
            ...
            sp.GetRequiredService<IUserSelfService>(),
            sp.GetRequiredService<FakeTimeProvider>()
        );
    }

    private async Task<(UserName UserName, NonValidatedPassword Correct, NonValidatedPassword Wrong)> SetupUserWithPasswordAsync(
        ...
        IUserSelfService userSelfService)
    {
        ...
        return (userName, correctSupplied, wrongSupplied);
    }

    [Fact]
    public async Task First_lockout_uses_first_escalating_duration()
    {
        var (sp, passwordAuth, selfService, userSelfService, timeProvider) = await CreateAsync(options =>
        {
        ...
        var (userName, correct, wrong) = await SetupUserWithPasswordAsync(selfService, userSelfService);

        // Trigger first lockout (2 failures)
        _ = (await passwordAuth.TryAuthenticateAsync(userName, wrong, _ct)).ShouldBeOfType<PasswordAuthenticationResult.Failure>();
        _ = (await passwordAuth.TryAuthenticateAsync(userName, wrong, _ct)).ShouldBeOfType<PasswordAuthenticationResult.Failure>();

        // Still blocked immediately after lockout
        _ = (await passwordAuth.TryAuthenticateAsync(userName, correct, _ct)).ShouldBeOfType<PasswordAuthenticationResult.Failure>();
        ...
        _ = (await passwordAuth.TryAuthenticateAsync(userName, correct, _ct)).ShouldBeOfType<PasswordAuthenticationResult.Success>();
    }
